...
#this plot is to be made in to a plots that for all three datasets only includes the data from 2008, so see if they all show the same trend as the GEM temperature data set.

ggplot() +
  # raw points + linear trend (historic)
  geom_point(data = nuuk21_04_dmi_temp_smoothed |> filter(year > 2007), aes(x = year, y = annual), size = 3, alpha = 0.6, color = "darkgreen") +
  geom_smooth(method = "lm", se = FALSE, color = "darkgreen") +

  # second dataset: note the data= and aes(x=Year, y=mean_temp)
  geom_point(data = nuuk25_08 |> filter(year > 2007),
             aes(x = Year, y = mean_temp),
             size = 3, alpha = 0.6, color = "red") +
  geom_smooth(data = nuuk25_08 |> filter(year > 2007),
              aes(x = Year, y = mean_temp),
              method = "lm", se = FALSE, color = "red") +
    geom_point(data = data_temp_annual_mean |> filter(year > 2007),
             aes(x = year, y = mean),
             size = 3, alpha = 0.6, color = "blue") +
  geom_smooth(data = data_temp_annual_mean |> filter(year > 2007),
              aes(x = year, y = mean),
              method = "lm", se = FALSE, color = "blue") +
