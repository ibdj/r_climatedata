...
format:
  html:
    ...
    css: styles.css
    # Uncomment and adjust the following for PDF output if needed
    # pdf:
...
# Add a month column as a factor with all months
df <- View_ClimateBasis_Nuuk_Data_Radiation_Photosyntetic_active_radiation_200_cm_5min_average_æmol_m2_sec210420241337080578 %>%
  mutate(month = factor(month(date, label = TRUE, abbr = FALSE), levels = month.name)) |> 
  filter(solar_elevation > 0)

# Plot
ggplot(df, aes(x = month, y = par_mmol_sec_m2)) +
  geom_boxplot() +
  xlab("Month") +
...
```{r}
par_highest_mean <- View_ClimateBasis_Nuuk_Data_Radiation_Photosyntetic_active_radiation_200_cm_5min_average_æmol_m2_sec210420241337080578 |> 
  filter(solar_elevation > 0) |> 
  group_by(year = year(date), month = lubridate::month(date, label = TRUE, abbr = FALSE)) |>  # Group by month name (e.g., "Jan")
