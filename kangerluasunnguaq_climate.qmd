...
#| fig-cap: !expr glue("Mean monthly distribution of types of precipitation (mm). Based on data from {min(data_pre_tem_joined_monthly$year)} to {max(data_pre_tem_joined_monthly$year)}. Sleet is defined as precipitation that fell when temperatures were between {sleet_lower}°C and {sleet_upper}°C.")
...
ggplot(data_degree_days_anomolies, aes(x = month_string, y = factor(year), fill = deviation)) +
  ...
  labs(
    title = "Degree Day Anomalies by Month-Year",
    x = "Month",
    y = "Year"
  ) +
  theme_minimal() +
...
```
# TO DOS
[ ] make all plots similar. Ref. mnds plots from NERO analysi.
