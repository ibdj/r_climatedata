...
#| fig-cap: !expr glue("Mean temperature pr summer month. Mean air temperature (°C) pr for {data_temp_monthly_mean$month_string[1]}, {data_temp_monthly_mean$month_string[2]}, and {data_temp_monthly_mean$month_string[3]} based on data from {min(data_temp$year)} to {max(data_temp$year)}. P-value for the regression i {round(model_temp_warmest_p_value,3)}. The mean of the warmests months across all years are {round(data_temp_mean_warmest,3)}°C.")
#| message: false
#| warning: false
...
ggplot() +
  ...
  labs(
    ...
    title = "Annual Mean Temperature with Interval Estimates"
  ) +
  theme_minimal()+
...
#| fig-cap: !expr glue("Monthly precipitation anomaly (mm). Stronger colors indicate large derivation from the month mean. Anomalies vary from {round(min(monthly_prep_anomalies$prep_anomaly),2)} to {round(max(monthly_prep_anomalies$prep_anomaly),2)}.")
...
## 5 year intervals precipitation
...
```{r}
#| label: data-precip-fiveyear-test
#| message: false
#| warning: false


data_precip_fiveyear_year <- data_precip_fiveyear |> 
  filter(year > 2007) |> 
  group_by(year, segment, seg_start, seg_end) |> 
  summarise(sum_precip = sum(pre_mm))

# Fit the model explicitly
model_precip <- lm(sum_precip ~ segment, data = data_precip_fiveyear_year)

# Get estimated marginal means with cld
emm_precip_fiveyear <- emmeans(model_precip, ~ segment)

cld(emm_precip_fiveyear)
...
#| fig-scap: "5 year intervals in precipitation means"
#| fig-cap: !expr glue("5 year intervals in precipitation means.")
#| message: false
#| warning: false

interval_mate <- data_precip_fiveyear_year |> 
  distinct(segment, seg_start, seg_end)

interval_lines_precip <- data_precip_fiveyear_year |>
  filter(year > 2007) |> 
  group_by(segment) |> 
  summarise(seg_mean = mean(sum_precip)) |> 
  left_join(interval_mate)
...
ggplot() +
  # Base: Yearly means (points)
  geom_point(
    data = data_precip_fiveyear_year,
  ...
  labs(
    ...
    title = "Annual Mean Precipitation with Interval Estimates"
  ) +
  theme_minimal()+
  scale_x_continuous(breaks=seq(min(data_precip_fiveyear_year$year), max(data_precip_fiveyear_year$year), by = 1))+
  scale_y_continuous(limits = c(0, NA), breaks=seq(0, max(data_precip_fiveyear_year$sum_precip), by = 100))
```
...
#| fig-cap: !expr glue("Yearly mean PAR from {min(par$year)} to {max(par$year)}. Calculations based on PAR values only when the sun is up (solar elevation > 0).")
#| message: false
#| warning: false
...
```
## Observations below 200

```{r}
#| label: fig-par-low-values
#| fig-scap: "Number of days with mean par below 200"
#| fig-cap: !expr glue("Number of days pr year with mean par below 200.")
#| message: false
#| warning: false
...
below_200 <- par |> 
  ...
  summarise(par_mmol_sec_m2 = max(par_mmol_sec_m2))

data_par_below_200 <- below_200 |> 
  group_by(year) |> 
  filter(par_mmol_sec_m2 < 200) |> 
  dplyr::summarize(below_200_count = n())

# Step 4: Plot
ggplot(data_par_below_200, aes(x = year, y = below_200_count)) +
  geom_point(size = 3, alpha = 0.7, color = "#076834") +
  scale_x_continuous(breaks=seq(min(below_200$year), max(below_200$year), by = 1))+
  geom_smooth(method = "lm", linetype = "dashed")+
  xlab("Year") +
  ylab("Number of measurements below 200")+
  theme_minimal()+
  stat_poly_eq(
      aes(label = paste(..eq.label.., ..rr.label.., ..p.value.label.., sep = "~~~")),
      formula = y ~ x,
...
# Fit the model
model_above <- lm(below_200_count ~ year, data = data_par_below_200)
...
#| fig-cap: !expr glue("Anomolies (difference from mean) of respective month. Stronger color indication bigger difference from monthly mean across entire period. Strongest color corresponding to anomaly of {round(min(par_monthly_anomalies$anomaly),1)} (darkest blue) - {round(max(par_monthly_anomalies$anomaly),1)} (darkest red).")
#| message: false
#| warning: false
...
Looking at the historic weather data from DMI from Nuuk. I.e. not the exact location of Kangerluarsunnguaq, but Nuuk. Comparing overall trends with data from [@cappelen2021; @jensen2024].
...
nuuk_21_04_dmi_temp_long <- nuuk_21_04_dmi_temp |> 
  pivot_longer(cols = c(jan:dec, mean_month, annual), names_to = "type", values_to = "temp")

filtered_data <- nuuk_21_04_dmi_temp_long |> filter(type != "annual", type != "mean_month")

ggplot(data = filtered_data, aes(x = year, y = temp)) +
  geom_point(size = 3, alpha = 0.3, color = "#076834") +
  geom_smooth(method = "lm", se = TRUE) +
  scale_x_continuous(breaks = seq(min(filtered_data$year), 2025, by = 20)) +
...
#| fig-cap: !expr glue("Annual historic temperature in °C comparing data from DMI 21-04, DMI 25-08 and GEM. Black line represent 15 year interval means for DMI 21-04.")
#| message: false
#| warning: false

ggplot(nuuk21_04_dmi_temp_smoothed_15yr |> 
         filter(annual < 10, annual > -10),
       aes(x = year, y = annual)) +
  # DMI 21_04: raw points + linear trend
  geom_point(data = nuuk_21_04_dmi_temp,
             aes(x = year, y = annual, colour = "DMI 21_04"),
             size = 3, alpha = 0.6) +
  geom_smooth(data = nuuk_21_04_dmi_temp,
              aes(x = year, y = annual, colour = "DMI 21_04"),
              method = "lm", se = FALSE) +

  # 15-year mean line (if you want it in legend, also map colour)
  geom_line(aes(y = seg_mean_temp, colour = "DMI 21-04 15y mean"), linetype = "solid",
            size = 1.1, ) +

  # DMI 25_08
  geom_point(data = nuuk25_08,
             aes(x = year, y = mean_temp, colour = "DMI 25_08"),
             size = 3, alpha = 0.6) +
  ...
  geom_point(data = data_temp_annual_mean,
             aes(x = year, y = mean, colour = "GEM data"),
             size = 3, alpha = 0.6) +
...
theme(
  ...
  legend.box = "horizontal"         # packs multiple legends horizontally
) +
guides(colour = guide_legend(nrow = 1))  # single row if needed
```

```{r}
#| label: fig-annual-temp-historic-from-2008
#| fig-scap: "Annual temperature trends only since 2008"
#| fig-cap: !expr glue("Annual historic temperature in °C only from 2008 to current. DMI 25-08 (Nuuk) and GEM data (Kangerluarsunguaq) corresponds very well with only a slight off set. ")
#| message: false
#| warning: false
...
[ ]download new PAR dataset.\par
[ ]make all the heat map the same axis.\par
