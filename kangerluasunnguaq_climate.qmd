...
```{r}
#| label: fig-par-month-highest-means2
#| fig-scap: "Mean of months with highest PAR"
#| fig-cap: !expr glue("Mean of months with highest PAR.")
# Plot

ggplot(par_highest_mean2, aes(x = year, y = par_mean)) +
  geom_point(size = 3) +
  geom_smooth(method = "lm", se = TRUE, color = "darkgray", linetype = "dashed") +
  #scale_x_continuous(
  #  breaks = 1:12,
...
```{r}
# Combined plot with both monthly & annual data
ggplot() +
  # Monthly data (colored points + dashed trend)
  geom_point(
    data = par_highest_mean, 
    aes(x = year, y = par_mean, color = factor(month_num)),
    size = 3,
  ...
  # Annual data (triangle markers + solid trend)
  geom_point(
    data = par_highest_mean2,
    aes(x = year, y = par_mean),
    shape = 17,  # Triangle marker
  ...
  geom_smooth(
    ...
    method = "lm",
    se = TRUE,
    color = "#E69F00",  # Distinct orange color
    linetype = "solid"
  ) +
  
  # Cosmetic improvements
  scale_color_viridis_d(name = "Month") +  # Better color palette
  labs(
    x = "Year",  # Corrected from original "Month" label
    y = "PAR (mmol/sec/m²)",
    title = "PAR Trends with Monthly Variation and Annual Means"
  ) +
  theme_minimal() +
