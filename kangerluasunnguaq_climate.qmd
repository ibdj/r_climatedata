...
nuuk21_04_dmi_temp_smoothed <- 
  ...
  arrange(year) |>
  mutate(
    annual_5yr = rollmean(annual, k = 20, fill = NA, align = "center")
  )
...
#| fig-cap: !expr glue("Mean air temperature (°C) pr year, from {min(gr_monthly_all_1784_2020$year)} to {max(gr_monthly_all_1784_2020$year)}. Yearly mean of {round(mean(gr_monthly_all_1784_2020$annual, na.rm= TRUE),1)}°C. Yearly mean ranges from {min(gr_monthly_all_1784_2020$annual, na.rm = TRUE)}°C to {max(gr_monthly_all_1784_2020$annual, na.rm = TRUE)}°C.")
...
ggplot() +
  # raw points + linear trend (historic)
  geom_point(data = nuuk21_04_after2007, aes(x = year, y = annual), size = 3, alpha = 0.6, color = "darkgreen") +
  geom_smooth(data = nuuk21_04_after2007, aes(x = year, y = annual), method = "lm", se = FALSE, color = "darkgreen") +

  # second dataset: note the data= and aes(x=Year, y=mean_temp)
  geom_point(data = nuuk25_08_after2007,
             aes(x = Year, y = mean_temp),
             size = 3, alpha = 0.6, color = "red") +
  geom_smooth(data = nuuk25_08_after2007,
              aes(x = Year, y = mean_temp),
              method = "lm", se = FALSE, color = "red") +
    geom_point(data = data_temp_annual_mean_after2007 |> filter(year > 2007),
             aes(x = year, y = mean),
             size = 3, alpha = 0.6, color = "blue") +
  geom_smooth(data = data_temp_annual_mean_after2007 |> filter(year > 2007),
              aes(x = year, y = mean),
              method = "lm", se = FALSE, color = "blue") +
