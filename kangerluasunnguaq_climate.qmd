...
nuuk_21_04_dmi_temp_long <- nuuk_21_04_dmi_temp |> 
  pivot_longer(cols = c(jan:dec, mean_month), names_to = "type", values_to = "temp")

ggplot(data = nuuk_21_04_dmi_temp_long |> filter(type == "mean_month"), aes(x = year, y = temp)) +
  geom_point(size = 1, alpha = 0.3, color = "#076834") +
  geom_smooth(method = "lm")+
  scale_x_continuous(
    breaks = seq(min(nuuk_21_04_dmi_temp_long$year),
                 max(2025), by = 20))+
  theme_minimal()
```
