...
install.packages("rmarkdown","pacman","tidyverse","yaml","xfun", dependencies = TRUE)
library(pacman)
pacman::p_load(ggplot2, tidyverse, janitor, lubridate, glue, hms,knitr, broom, ggpmisc, yaml)
...
#| fig-cap: !expr glue("Mean air temperature pr year (°C) from {min(data_temp_annual_mean$year)} to {max(data_temp_annual_mean$year)}. Data from 2007 is excluded because data is only from October, November and December. Blue dotted line indicate over all mean of {year_mean}°C. Dashed gray line is the trend line.")
#| message: false
#| warning: false

ggplot(data_temp_annual_mean, aes(x = year, y = mean))+
  geom_smooth(method = lm, color = "darkgray", se = F, linetype = "dashed")+
  geom_point(size = 3, color = "darkgreen")+
  #geom_line(size = 0.25, color = "black", alpha = 0.2)+
  scale_x_continuous(breaks=seq(min(data_temp_annual_mean$year), max(data_temp_annual_mean$year), by = 1))+
  geom_hline(yintercept=year_mean, linetype="dotted", color = "blue", size = 1)+
  ...
  theme(plot.margin = unit(c(-5, 1, -4, 1), "mm"))
```
...
geom_hline(yintercept = precip_year_mean, linetype="dotted", color = "blue", linewidth =1)+
  geom_smooth(method = lm, color = "darkgray", size = 1, se = F, linetype = "dashed")+
...
ggplot(data_prep_anomaly_abs, aes(x = year, y = abs_anomo)) +
  ...
  xlab("") +
  ylab("Absolute anomolies, precipitation (mm)") +
...
max_y <- max(par_yearly_mean$mean, na.rm = TRUE)
min_y <- min(par_yearly_mean$mean, na.rm = TRUE)
...
ggplot(par_yearly_mean, aes(x = year, y = mean)) +
  ...
  geom_point(size = 3, color = "darkgreen")+
  labs(x = "", y = "PAR (mmol/sec/m2)", title = "")+
  geom_hline(yintercept=par_mean, linetype="dotted", color = "blue", size = 1)+
  stat_poly_eq(
      aes(label = paste(..p.value.label.., ..rr.label..,..eq.label..,  sep = "~~~")),
      formula = y ~ x,
      parse = TRUE
...
data_degree_days_mean <- mean(data_degree_days_count$n_days[data_degree_days_count$year > 2007], na.rm = TRUE)
...
ggplot(data_degree_days_count |> filter(year > 2007), aes(x = year, y = n_days)) +
  ...
  scale_x_continuous(breaks=seq(min(data_degree_days$year), max(data_degree_days$year), by = 1))+
    geom_hline(yintercept=data_degree_days_mean, linetype="dotted", color = "blue", size = 1)+
  #ylim(0,255)+
   stat_poly_eq(
      aes(label = paste(..p.value.label.., ..rr.label..,..eq.label..,  sep = "~~~")),
      formula = y ~ x,
